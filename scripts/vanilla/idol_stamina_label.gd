...
func native_update_data():
	if GlobalFunc._check_if_character_stage():
		var now_stamina = GlobalVar.module_temp_data["character"][GlobalVar.in_game_data["select_one"]]["detail"]["stamina"]
		var max_stamina = GlobalVar.module_temp_data["character"][GlobalVar.in_game_data["select_one"]]["detail"]["max_stamina"]
